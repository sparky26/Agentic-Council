...
from council.config.settings import get_settings, ModelConfig
from council.llm.base_client import LLMClient
from council.llm.groq_client import GroqLLMClient
...
def get_llm_client() -> LLMClient:
    ...
    Currently this is a GroqLLMClient, but higher-level code should depend on
    the LLMClient interface, not on the concrete type.
    """
    global _LLM_CLIENT
    if _LLM_CLIENT is None:
        settings = get_settings()
        _LLM_CLIENT = GroqLLMClient.from_settings(settings)
    return _LLM_CLIENT
